...
$('#kiraciekle').submit(function(event){
...
function dairesil(id){
			if(confirm("Devam ederseniz Daire veritabanından kalıcı olarak silinecektir!")){
				$.ajax({ 
				type:'post',
				url:'./posts.php?islem=20',
				data:{"daireid":id},
				success:function(cevap){
				alert(cevap);
				tumunugetir();
  ...
  function kiracicikar(did,kulid){
	  	$.ajax({ 
						type:'post',
						url:'./posts.php?islem=19',
						data:{"kulid":did,"daireid":kulid},
						success:function(cevap){
							alert(cevap);
							$('#model').modal('hide');
						}	
